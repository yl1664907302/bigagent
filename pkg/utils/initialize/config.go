...
func InitConfig() {
	...
	defaultConfigPath := *base.FullDir + "\\configs\\config.yaml"
...
func setDefaultBaseConfig() {
	...
	*/
	...
	viper.SetDefault("log.format", "text")                                        //日志记录格式；（选项：json、text）
	viper.SetDefault("log.runtimeLogFile", *base.FullDir+"/logs/log_runtime.log") //日志输出位置
	viper.SetDefault("log.accessLogFile", *base.FullDir+"/logs/log_access.log")   //日志输出位置
}

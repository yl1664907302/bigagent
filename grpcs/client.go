...
// newToken 创建一个新的 TokenCredentials 实例
...
func InitClient(host string, token string) (*grpc.ClientConn, error) {
	...
	conn, err := grpc.DialContext(
		ctx,
		host,
		grpc.WithPerRPCCredentials(NewGrpcToken(token)),          //设置token
		grpc.WithTransportCredentials(insecure.NewCredentials()), //Credential即使为空，也必须设置
		grpc.WithBlock(), //grpc.WithBlock()直到连接真正建立才会返回
...
func GrpcStandPush(conn *grpc.ClientConn) {
	...
	request := grpc_server.SmpData{
		...
		MachineType:  data.Virtual,
		Os:           data.Os,
		Kernel:       data.Kernel,
		Arch:         data.Arch,
		DiskUse:      data.Disk_use,
		CpuUse:       data.Cpu_use,

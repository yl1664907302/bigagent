...
func (p *PostVeops) CreateMachineUUID() {
	if !p.k {
		...
		if err != nil {
			utils2.DefaultLogger.Error("唯一键数据更新失败：", veopsdata)
			return
		}
		utils2.DefaultLogger.Info("唯一键数据创建成功：", veopsdata)
		p.k = true
		oIdStr := strconv.FormatFloat((*responses)["ci_id"].(float64), 'f', -1, 64)
		utils2.DefaultLogger.Info("创建实例成功ci_id为：", oIdStr)
	}
}
...
func (p *PostVeops) UpdateMachineData() {
	...
	if err != nil {
		utils2.DefaultLogger.Error("主机数据更新失败：", veopsdata)
		return
	}
	utils2.DefaultLogger.Info("数据更新成功：", veopsdata)
}
...
func (p *PostVeops) CheckClient() {
	...
	if err != nil {
		utils2.DefaultLogger.Error("CMDB客户端连接串配置错误", err)
	...
	if err != nil {
		...
		utils2.DefaultLogger.Error("CMDB客户端连接检测请求发送失败：", err)
		panic("CMDB客户端连接检测请求发送失败")
	}
	defer func(Body io.ReadCloser) {
		err := Body.Close()
		if err != nil {
			utils2.DefaultLogger.Error("CMDB客户端连接关闭失败：", err)
		}
	}(resp.Body)

	// 读取并打印响应体
	if resp.StatusCode != 200 {
		utils2.DefaultLogger.Error("CMDB客户端连接检测失败：")
		panic("CMDB客户端连接检测失败,错误码为：" + strconv.Itoa(resp.StatusCode))
	}
	_, err = ioutil.ReadAll(resp.Body)
	if err != nil {
		//fmt.Println("Error reading response body:", err)
		utils2.DefaultLogger.Error("读取响应结果时异常：", err)
...
func (p *PostVeops) Flurl(apiPath string, params model.VeopsData) (string, error) {
	...
	if err != nil {
		utils2.DefaultLogger.Error("密钥对hash加密失败：", err)
	}
	hashCMDBSecret, err := utils2.JSONToFormData(params)
...
func HashCMDBSecret(apiPath string, params model.VeopsData) error {
	...
	ks, vs := utils2.GetNonEmptyFields(params)
	...
	//  移除指定key值
	utils2.RemoveString(vs, "_secret")
	utils2.RemoveString(vs, "_key")
...
func (p *PostVeops) PostCMDBci(params *model.VeopsData) (*response.ResultResponse, error) {
	...
	body, err := ioutil.ReadAll(resp.Body)
	if err != nil {
		utils2.DefaultLogger.Error("CMDB客户端响应体读取异常：", err)
		return nil, err
	}
	if resp.StatusCode != 200 {
		errorData, err := response.ResponseErrorData(resp.StatusCode, body)
	...
	utils2.DefaultLogger.Info("新增数据返回结果为：", jsonData)
	return jsonData, nil
}
...
func (p *PostVeops) PutCMDBci(params *model.VeopsData, ciId string) (*response.ResultResponse, error) {
	...
	if err != nil {
		utils2.DefaultLogger.Error("CMDB客户端连接请求发送失败：", err)
	}
	resp, err := client.Do(req)
	if err != nil {
		utils2.DefaultLogger.Error("CMDB客户端连接异常：", err)
	}
